...
\begin{frame}{Nützliche Files und Adressen}
 \begin{tabular}{ll}
  ...
  \url{https://cat-test.eduroam.org} & sollte gehen
     \footnote{\url{https://cat.eduroam.org} funktioniert nicht}\\
  \cod{connect.txt} & zeigt die Schritte     
 \end{tabular}
\end{frame}

\subsection{Das sollte funktionieren}

\begin{frame}{Zwei Verbindungen:LAN/WLAN}{für \target}
 \begin{description}[WLAN]
  \item[WLAN] mit der Welt
   \begin{itemize}
    \item \target ist Client
   \end{itemize}
  \item[LAN] mit den \host im gleichen Netz
   \begin{itemize}
    \item \target ist Server
   \end{itemize}
 \end{description}
 \remark{\cod{connect.txt} zeigt die Befehle} 
\end{frame}

\begin{frame}{LAN}
 \begin{block}{\host}
 \begin{itemize}
  \item \cod{ifconfig eth0:0 192.168.1.1 up}
  \remark{die anderen Verbindungen bleiben}
 \end{itemize}
 \end{block}
 \begin{block}{\target}
  \begin{itemize}
...
\begin{frame}{WLAN \target}{Manuell}
 \begin{description}[Konfiguration]
  \item[Background] \cod{wpa\_supplicant -D wext -i wlan0 -c{\em wpa.conf} -B}
  \item[Foreground] \cod{wpa\_cli}
  \item[Konfiguration] \cod{wpa\_supplicant.conf}
  \remark{Passwort noch in Klartext}
 \end{description}
\end{frame}

\begin{frame}{WLAN \target}{netctl}
 \begin{itemize}
  \item {\footnotesize\url{https://wiki.archlinux.org/index.php/netctl}}
  \item Passwort mindestens verschleiern
 \end{itemize}
\end{frame}
\end{document}
