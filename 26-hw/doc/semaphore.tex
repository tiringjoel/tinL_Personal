...
\begin{itemize}
 \item put
  \begin{itemize}
   \item füge hinten an \cod{put}: data: Zustand des Schalters: \cod{on$|$off}
   \item öffne Semaphore
  \end{itemize}
 \item get
 \begin{itemize}
...
\begin{frame}{Die Semaphore-Queue}
 \begin{itemize}
  \item \cod{struct} für Semaphore-Queue
  \item \cod{init\_} 
  \item \cod{put\_}
  \item \cod{get\_}
  \item Dynamisches Memory
...
\begin{frame}[fragile]{TODO's}{Im Zusammenhang mit den Interrups}
 \begin{itemize}
  \item mehr Informationen in \cod{buf} von \cod{get\_swi} zurückgeben: 
  \begin{lstlisting}
  struct SWIInfo
  {
   time_t   when;
